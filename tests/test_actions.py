...
class TestConnect(BaseTest):
    ...
    def test_get_or_create_updates_credentials_when_account_exists(self):
        """SK-821 regression: authorization_details must not be silently dropped when account exists."""
        import uuid
        test_id = f"test_sk821_{uuid.uuid4().hex[:8]}"
        ...
        try:
            result = self.scalekit_client.connect.get_or_create_connected_account(
                connection_name="GMAIL",
                identifier=test_id,
                authorization_details=initial_auth,
            )
            self.assertIsNotNone(result)
            self.assertEqual(result.connected_account.identifier, test_id)

            # Calling again with new auth details must trigger an update, not silently drop credentials.
            result2 = self.scalekit_client.connect.get_or_create_connected_account(
                connection_name="GMAIL",
                identifier=test_id,
                authorization_details=updated_auth,
            )
            self.assertIsNotNone(result2)
            self.assertIsNotNone(result2.connected_account)
            self.assertEqual(result2.connected_account.identifier, test_id)
    ...
    def test_get_or_create_returns_existing_when_no_auth_details(self):
        """When no authorization_details provided and account exists, return as-is (no update)."""
        try:
            result = self.scalekit_client.connect.get_or_create_connected_account(
                connection_name=self.test_connection_name,
                identifier=self.test_identifier,
    ...
    def test_upsert_connected_account_method_exists(self):
        """upsert_connected_account alias must be present on the actions client."""
        self.assertTrue(hasattr(self.scalekit_client.connect, 'upsert_connected_account'))
        self.assertTrue(callable(self.scalekit_client.connect.upsert_connected_account))

    def test_upsert_connected_account_validates_params(self):
        """upsert_connected_account must mirror get_or_create validation."""
        with self.assertRaises(ValueError):
            self.scalekit_client.connect.upsert_connected_account(
                connection_name="",
                identifier="user_123",
            )
    ...
    def test_upsert_connected_account_creates_and_updates(self):
        """upsert_connected_account behaves identically to get_or_create_connected_account."""
        import uuid
        test_id = f"test_upsert_{uuid.uuid4().hex[:8]}"

        auth = {
            "oauth_token": {
                "access_token": "upsert_access_token",
                "refresh_token": "upsert_refresh_token",
                "scopes": ["read"],
            }
        }
        ...
        result = None
        try:
            result = self.scalekit_client.connect.upsert_connected_account(
                connection_name="GMAIL",
                identifier=test_id,
